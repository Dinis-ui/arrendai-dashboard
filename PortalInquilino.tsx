...
export default function PortalInquilino() {
  ...
  const carregarNotificacoes = async () => {
    ...
    if (!token) return;
    try {
      const res = await fetch('https://arrendai-dashboard.onrender.com](https://arrendai-dashboard.onrender.com/api/users/notificacoes/', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleToggleNotificacoes = async () => {
    ...
    if (newState && naoLidas > 0) {
      const token = localStorage.getItem('accessToken') || sessionStorage.getItem('accessToken');
      try {
        await fetch('https://arrendai-dashboard.onrender.com](https://arrendai-dashboard.onrender.com/api/users/notificacoes/lidas/', {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${token}` }
        });
  ...
  const guardarConta = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem('accessToken') || sessionStorage.getItem('accessToken');
    try {
      const response = await fetch('https://arrendai-dashboard.onrender.com](https://arrendai-dashboard.onrender.com/api/users/me/', {
        method: 'PATCH',
        headers: {
  ...
  const atualizarPassword = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem('accessToken') || sessionStorage.getItem('accessToken');
    try {
      const response = await fetch('https://arrendai-dashboard.onrender.com](https://arrendai-dashboard.onrender.com/api/users/change-password/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  useEffect(() => {
    const buscarImoveis = async () => {
      ...
      try {
        const response = await fetch('https://arrendai-dashboard.onrender.com](https://arrendai-dashboard.onrender.com/api/users/propriedades/', {
          headers: { 'Authorization': `Bearer ${token}` }
        });
  ...
  useEffect(() => {
    if (activeTab === 'Minhas Candidaturas') {
      const carregarAsMinhasCandidaturas = async () => {
        ...
        if (!token) return;
        try {
          const res = await fetch('https://arrendai-dashboard.onrender.com](https://arrendai-dashboard.onrender.com/api/tenancies/applications/', {
            headers: { 'Authorization': `Bearer ${token}` }
          });
  ...
  useEffect(() => {
    if (activeTab === 'As Minhas Rendas') {
      const carregarRendas = async () => {
        ...
        if (!token) return;
        try {
          const res = await fetch('https://arrendai-dashboard.onrender.com](https://arrendai-dashboard.onrender.com/api/tenancies/tenancies/', {
            headers: { 'Authorization': `Bearer ${token}` }
          });
  ...
  const processarPagamento = async (e: React.FormEvent) => {
    ...
    setTimeout(async () => {
      const token = localStorage.getItem('accessToken') || sessionStorage.getItem('accessToken');
      try {
        const response = await fetch(`https://arrendai-dashboard.onrender.com](https://arrendai-dashboard.onrender.com/api/tenancies/tenancies/${rendaAPagar.id}/pay/`, {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${token}` }
        });
  ...
  const handleApagarCandidatura = async (id: number) => {
    ...
    if (window.confirm("Tens a certeza que queres retirar a tua candidatura?")) {
      try {
        const response = await fetch(`https://arrendai-dashboard.onrender.com](https://arrendai-dashboard.onrender.com/api/tenancies/applications/${id}/`, {
          method: 'DELETE',
          headers: { 'Authorization': `Bearer ${token}` }
        });
