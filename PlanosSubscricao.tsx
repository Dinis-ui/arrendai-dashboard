...
export default function PlanosSubscricao() {
  ...
  const carregarDados = async () => {
    ...
    try {
      ...
      const resPlanos = await fetch('https://arrendai-dashboard.onrender.com](https://arrendai-dashboard.onrender.com/api/users/planos/', { 
        headers: { 'Authorization': `Bearer ${token}` } 
      });
  ...
  const processarPagamentoDireto = async (planoId: number, e?: React.FormEvent) => {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        const response = await fetch('https://arrendai-dashboard.onrender.com](https://arrendai-dashboard.onrender.com/api/users/subscribe/', {
          method: 'POST',
...
const simularMudancaPlano = async (planoId: number) => {
  const token = localStorage.getItem('accessToken') || sessionStorage.getItem('accessToken');
  await fetch('https://arrendai-dashboard.onrender.com](https://arrendai-dashboard.onrender.com/api/users/me/subscribe/', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    body: JSON.stringify({ plano_id: planoId })
  });
