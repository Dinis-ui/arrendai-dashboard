...
export default function Propriedades({ onMudarParaAnuncios, onMudarParaMensagens }: PropriedadesProps) {
  ...
  const handleCriarAnuncio = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (propriedadeSelecionadaId && token) {
      try {
        const response = await fetch(`https://arrendai-dashboard.onrender.com](https://arrendai-dashboard.onrender.com/api/users/propriedades/${propriedadeSelecionadaId}/`, {
          method: 'PATCH',
  ...
  if (propriedadeSelecionadaId !== null) {
    ...
    const handleEncerrarContrato = async (e: React.FormEvent) => {
      ...
      try {
        const response = await fetch(`https://arrendai-dashboard.onrender.com](https://arrendai-dashboard.onrender.com/api/tenancies/tenancies/${prop.contratoId}/close/`, {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${token}` }
        });
